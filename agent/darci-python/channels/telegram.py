...
from darci.config.schema import TelegramConfig, FLASH_MODEL


_USERS_FILE = Path.home() / ".darci-python" / "users.json"


def _track_user(user) -> dict:
    """Persist a Telegram user's info to ~/.darci-python/users.json. Returns {is_new, record}."""
    try:
...
class TelegramChannel(BaseChannel):
    ...
    async def _on_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await update.message.reply_text(
            f"👋 Hi {user.first_name}! I'm darci.\n\n"
        ...
        if media_file and self._app:
            try:
                ...
                if media_type in ("voice", "audio"):
                    media_dir = Path.home() / ".darci-python" / "media" / "voicemessage" / str(user.id)
                else:
                    media_dir = Path.home() / ".darci-python" / "media"
