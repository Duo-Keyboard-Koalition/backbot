...
Self-contained Go implementation of the darci agent loop.

## Status

**Build:** ✅ Passing (`go build ./...`)
**Executable:** ✅ Working (`darci.exe`)
**Last Updated:** 2026-03-07

## Core value

darci-Go ADK defaults to local, self-contained behavior:
- no external model dependency required to run
- workspace-scoped file tools with path escape protection
- single binary execution path (`go run ./cmd/darci-go`)
...
```bash
go build -o darci.exe ./cmd/darci-go
./darci.exe help
```

Optional:
- `darci_GO_WORKSPACE=/path/to/workspace` to limit file tools.
...
- `darci/agent/`: agent, subagent, skills, and tools management
- `darci/session/`: session management for user conversations
- `darci/cli/`: command-line interface

## Project Structure

```
darci-go/
├── cmd/darci-go/     # Main executable
...
Configuration is stored in `~/.darci-go/config.json`.
