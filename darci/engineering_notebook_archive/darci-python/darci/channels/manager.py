...
from darci.config.schema import Config, TTS_MODEL

_ELEVENLABS_TTS_URL = "https://api.elevenlabs.io/v1/text-to-speech/{voice_id}"
_TTS_WORKSPACE = Path.home() / ".darci-python" / "workspace"
...
async def _elevenlabs_tts(text: str, api_key: str, voice_id: str, chat_id: str = "") -> Path | None:
    ...
    try:
        ...
        if chat_id:
            tts_dir = Path.home() / ".darci-python" / "media" / "voicemessage" / str(chat_id)
        else:
...
class ChannelManager:
    ...
    def _init_channels(self) -> None:
        ...
        if self.config.channels.telegram.enabled:
            try:
                from darci.channels.telegram import TelegramChannel
        ...
        if self.config.channels.email.enabled:
            try:
                from darci.channels.email import EmailChannel
        ...
        if self.config.channels.slack.enabled:
            try:
                from darci.channels.slack import SlackChannel
