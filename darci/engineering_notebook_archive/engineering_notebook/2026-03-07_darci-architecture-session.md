...
Analyze existing codebase (backend/, darci/, tailbridge/) to determine how to build a DarCI-based AI agent project manager. Map every component, trace every protocol, design the integration architecture, and offload all context into persistent logs.
...
### 2. darci/darci-python/ — Agent Framework (= DarCI Runtime)

Ultra-lightweight (3,935 LOC) Python AI agent framework. **DarCI IS DarCI** when configured with DARCI tools and system prompt.
...
**Config structure (~/.darci/config.json):**
```json
{
...
### 4. darci/darci-brainstorm/ — Design Documentation
...
### ADR-001 — DarCI IS DarCI

**Status:** ACCEPTED

**Decision:** DarCI is not a separate process. DarCI is configured with DARCI-specific tools, system prompt, and skills. Running `darci gateway` with the darci skill loaded = running DarCI. No new agent loop class is created.
...
**Consequences:** The `darci/` package contains only tools, models, state store, system prompt, and skill file. It is imported by DarCI, not the other way around.
...
|---|---|---|
| Driver | DarCI/DarCI | Owns tasks.json, sets goals, dispatches via taila2a |
...
| Consulted | DarCI subagents | Spawned for analysis before task assignment |
...
**Decision:** Each agent's sentinel stream is monitored by a persistent `asyncio.Task` (not a DarCI subagent). Reports back via `bus.publish_inbound(channel="system")`.

**Rationale:** DarCI subagents are designed for one-shot LLM tasks. Sentinel monitoring is a long-lived WebSocket event stream. Using asyncio.Task directly mirrors the pattern in DarCI's `SubagentManager` and `tailbridge/taila2a/internal/services/` without the LLM overhead.
...
| 4 | darci.agent.tools.base.Tool | Base class for all DarCI tools |
| 5 | darci.agent.loop.AdkAgentLoop | Core agent runtime |
...
├── state/
│   └── store.py          # TaskStore: JSON I/O for ~/.darci/darci/
├── tools/
