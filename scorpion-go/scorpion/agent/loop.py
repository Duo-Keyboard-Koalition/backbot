...
class AdkAgentLoop:
    ...
    async def _run_agent_adk(
        ...
        # ADK LlmAgent reads GOOGLE_API_KEY from env — inject from config if not set.
        import os as _os
        _api_key = getattr(self.provider, "api_key", "") or ""
        if _api_key and not _os.environ.get("GOOGLE_API_KEY"):
            _os.environ["GOOGLE_API_KEY"] = _api_key
