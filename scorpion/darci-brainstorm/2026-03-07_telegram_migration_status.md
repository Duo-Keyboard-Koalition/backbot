...
**Errors:**
```
scorpion\agent\context.go:6:2: package scorpion-go/scorpion/adk is not in std
scorpion\channels\base.go:6:2: package scorpion-go/scorpion/bus is not in std
scorpion\cli\commands.go:11:2: package scorpion-go/scorpion/agent is not in std
```

**Root cause:** Import paths reference `scorpion-go/scorpion/*` but packages are in `internal/adk/`.

---

## Next Steps

1. **Fix import paths** - Change `scorpion-go/scorpion/adk` → `scorpion-go/internal/adk`
...
go run ./cmd/scorpion-go gateway
```
