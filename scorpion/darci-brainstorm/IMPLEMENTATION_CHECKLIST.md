...
- [ ] Install Scorpion framework
  ```bash
  cd ../scorpion-python
  pip install -e .
  ```

- [ ] Initialize Scorpion configuration
  ```bash
  scorpion onboard
  ```

- [ ] Add Gemini API key to `~/.scorpion/config.json`
  ```json
  {
    "providers": {
      "gemini": {
...
- [ ] Create DarCI config file
  ```bash
  cat > ~/.scorpion/darci/config.yaml << EOF
  darci:
...
- [ ] Create task store schema (`~/.scorpion/darci/tasks.json`)
  ```json
  {
    "version": "1.0",
    "tasks": {},
...
- [ ] Implement `build_run` tool
  - [ ] Support scorpion-go target
  - [ ] Support scorpion-python target
...
- [ ] Create file watcher for scorpion-go
  - [ ] Watch `*.go` files
  - [ ] Watch `go.mod`, `go.sum`
  - [ ] Ignore `vendor/`, `.git/`

- [ ] Create file watcher for scorpion-python
  - [ ] Watch `*.py` files
...
- [ ] Implement import path fixer
  - [ ] Detect `scorpion-go/scorpion/*` imports
  - [ ] Replace with `scorpion-go/internal/*`
  - [ ] Run `go mod tidy`
  - [ ] Verify build
...
- [ ] Create startup script
  ```bash
  #!/bin/bash
  cd ~/repos/sentinelai/scorpion/scorpion-python
...
- [ ] Create systemd service (Linux)
  ...
  [Service]
  ExecStart=/path/to/scorpion gateway
  Restart=always
  User=darcy
