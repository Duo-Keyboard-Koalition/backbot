...
# Single command
scorpion agent -m "Create tasks for the Go Telegram migration"
...
"Build scorpion-go and fix any errors"
...
```bash
export SCORPION_WORKSPACE=/path/to/workspace
```

### DarCI Config

```yaml
# ~/.scorpion/darci/config.yaml
darci:
...
| Build status | "Build scorpion-go" | ≥ 95% success |
...
```
User: "Build scorpion-go"

DarCI:
  🔧 Building... ❌ Failed
  
  3 import errors found:
    - scorpion/adk → internal/adk
    - scorpion/bus → internal/bus
    - scorpion/agent → internal/agent
