...
- **Role**: Commit and push to the private `.darci` repo
- **Key**: Uses dedicated `~/.darci/.ssh/darci` key — never the system SSH key
...
Managed with `~/.darci/commands/svc`:

```
~/.darci/commands/svc status       # show all services
~/.darci/commands/svc start darci
~/.darci/commands/svc stop darci
~/.darci/commands/svc logs darci # tail logs
```
