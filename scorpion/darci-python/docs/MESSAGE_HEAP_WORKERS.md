...
Turn 2: User sends any message
  → build_context_block() injected into prompt:
      [COMPLETED] video generation "a cat" → /home/.../.darci/media/videos/video_....mp4
...
#### `darci/adk/pending.py` — Message Heap
...
#### `darci/adk/workers.py` — Direct API Workers
...
2. Saves output to `~/.darci/media/{kind}/`
...
#### `darci/adk/tools.py` — Tools as Launchers
...
# Ensure GEMINI_API_KEY is set (or present in ~/.darci/config.json)
```
...
```bash
darci -m "generate a video of a sunset over the ocean"
```
...
| Kind | Path |
|------|------|
| Images | `~/.darci/media/images/image_YYYYMMDD_HHMMSS.png` |
| Videos | `~/.darci/media/videos/video_YYYYMMDD_HHMMSS.mp4` |
| Music | `~/.darci/media/music/music_YYYYMMDD_HHMMSS.wav` |
| Speech | `~/.darci/media/voicemessage/speech_YYYYMMDD_HHMMSS.wav` |
