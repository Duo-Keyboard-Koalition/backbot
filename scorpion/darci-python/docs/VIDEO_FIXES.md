...
This document describes the fixes applied to the video generation functionality in darci. The changes fix critical bugs where user-specified parameters were not being passed to the Google Veo API, and add new video understanding capabilities.
...
**Files Fixed:**
- `darci/agent/tools/creative.py` (line 268-273)
- `darci/adk/tools.py` (line 788-794)
...
```python
async def execute(self, prompt: str, duration: int = 8, ...):
    from darci.adk.tools import generate_video
...
- **Generated videos**: `~/.darci/media/videos/video_YYYYMMDD_HHMMSS.mp4`
- **Generated images**: `~/.darci/media/images/image_YYYYMMDD_HHMMSS.png`
- **Generated music**: `~/.darci/media/music/music_YYYYMMDD_HHMMSS.wav`
...
```bash
# Start the agent
darci agent -m "Generate a 4-second video of a cat walking in 9:16 aspect ratio"
...
darci agent -m "Analyze this video: /path/to/my/video.mp4"

# Or ask questions about a video
darci agent -m "What happens in this video? /path/to/video.mov"

# YouTube URL support
darci agent -m "Summarize this video: https://youtube.com/watch?v=..."
```
...
|------|---------|---------------|
| `darci/agent/tools/creative.py` | Fixed duration param, consolidated code | ~60 removed |
| `darci/adk/tools.py` | Fixed duration parameter | 1 |
| `darci/providers/gemini_provider.py` | Added video understanding | ~150 added |
