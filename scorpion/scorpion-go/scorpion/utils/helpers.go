// Package utils provides utility functions for scorpion-go.
package utils
...
// GetDataPath returns the ~/.scorpion-go data directory.
func GetDataPath() (string, error) {
	homeDir, err := os.UserHomeDir()
	if err != nil {
		return "", err
	}
	path := filepath.Join(homeDir, ".scorpion-go")
	return path, EnsureDir(path)
}

// GetWorkspacePath resolves and returns the workspace path.
// Defaults to ~/.scorpion-go/workspace if not specified.
func GetWorkspacePath(workspace string) (string, error) {
	if workspace == "" {
		...
		workspace = filepath.Join(homeDir, ".scorpion-go", "workspace")
	}
