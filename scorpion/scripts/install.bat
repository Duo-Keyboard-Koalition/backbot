...
REM Build Go bot
echo Building scorpion-go...
go build -o scorpion-go.exe ./cmd/scorpion-go
if %errorlevel% neq 0 (
...
xcopy /E /I /Y "%~dp0..\scorpion-python\docs\*.md" "%WORKSPACE%\" 2>nul
...
echo @echo off.
echo cd /d "%%~dp0..\..\..\repos\sentinelai\scorpion\scorpion-python".
echo uv run scorpion agent.
echo pause. > "%SHORTCUT_DIR%\scorpion-python.bat"

echo @echo off.
echo cd /d "%%~dp0..\..\..\repos\sentinelai\scorpion\scorpion-go".
echo scorpion-go.exe.
echo pause. > "%SHORTCUT_DIR%\scorpion-go.bat"

echo @echo off.
echo cd /d "%%~dp0".
echo uninstall.bat.
echo pause. > "%SHORTCUT_DIR%\Uninstall scorpion.bat"
...
set "USER_BIN=%USERPROFILE%\scorpion-bin"
if not exist "%USER_BIN%" mkdir "%USER_BIN%"

REM Get absolute paths
set "SCORPION_DIR=%~dp0.."
set "SCORPION_PYTHON_DIR=%SCORPION_DIR%\scorpion-python"
set "SCORPION_GO_DIR=%SCORPION_DIR%\scorpion-go"

REM Create Python launcher that activates venv automatically
(
echo @echo off
echo REM scorpion-python launcher - Windows
echo cd /d "%SCORPION_PYTHON_DIR%"
echo call "%SCORPION_PYTHON_DIR%\.venv\Scripts\activate.bat"
echo python -m scorpion agent %%*
) > "%USER_BIN%\scorpion-python.bat"

REM Create Go launcher
(
echo @echo off
echo REM scorpion-go launcher - Windows
echo cd /d "%SCORPION_GO_DIR%"
echo scorpion-go.exe %%*
) > "%USER_BIN%\scorpion-go.bat"
...
echo cd /d "%~dp0"
echo uninstall.bat %%*
) > "%USER_BIN%\uninstall-scorpion.bat"
...
echo   From anywhere: scorpion-python or scorpion-go (after adding to PATH)
echo   From Start Menu: scorpion-python or scorpion-go
echo   Manual: cd scorpion-python ^&^& uv run scorpion agent -m "Hello!"
echo           cd scorpion-go ^&^& scorpion-go.exe
echo.
