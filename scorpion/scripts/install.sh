#!/bin/bash
# scorpion Install Script for Linux/macOS
...
echo "🐈 scorpion - Installation (Linux/macOS)"
echo "========================================="
...
go build -o scorpion-go ./cmd/scorpion-go
if [ $? -ne 0 ]; then
...
CONFIG_DIR="$HOME/.config/scorpion"
mkdir -p "$CONFIG_DIR"
...
WORKSPACE="$HOME/scorpion-workspace"
mkdir -p "$WORKSPACE"

# Copy bootstrap files
cp -r "$SCORPION_DIR/scorpion-python/docs/"*.md "$WORKSPACE/" 2>/dev/null || true
...
# Get absolute paths
SCORPION_PYTHON_DIR="$(cd "$SCORPION_DIR/scorpion-python" && pwd)"
SCORPION_GO_DIR="$(cd "$SCORPION_DIR/scorpion-go" && pwd)"
SCRIPTS_DIR="$(cd "$SCRIPT_DIR" && pwd)"

# Create Python launcher that activates venv automatically
cat > "$BIN_DIR/scorpion-python" << LAUNCHER
#!/bin/bash
# scorpion-python launcher - Linux/macOS
cd "$SCORPION_PYTHON_DIR"
...
# Run scorpion agent
python -m scorpion agent "\$@"
LAUNCHER
chmod +x "$BIN_DIR/scorpion-python"

# Create Go launcher
cat > "$BIN_DIR/scorpion-go" << LAUNCHER
#!/bin/bash
# scorpion-go launcher - Linux/macOS
cd "$SCORPION_GO_DIR"
./scorpion-go "\$@"
LAUNCHER
chmod +x "$BIN_DIR/scorpion-go"

# Create uninstall launcher
cat > "$BIN_DIR/uninstall-scorpion" << LAUNCHER
#!/bin/bash
# scorpion uninstall launcher
cd "$SCRIPTS_DIR"
./uninstall.sh "\$@"
LAUNCHER
chmod +x "$BIN_DIR/uninstall-scorpion"
...
if [[ ":$PATH:" != *":$HOME/.local/bin:"* ]]; then
    ...
    if ! grep -q ".local/bin" "$SHELL_RC" 2>/dev/null; then
        echo "" >> "$SHELL_RC"
        echo '# Added by scorpion installer' >> "$SHELL_RC"
...
echo "  From anywhere: scorpion-python or scorpion-go"
echo "  Manual: cd scorpion-python && uv run scorpion agent -m \"Hello!\""
echo "          cd scorpion-go && ./scorpion-go"
echo ""
if [[ ":$PATH:" != *":$HOME/.local/bin:"* ]]; then
