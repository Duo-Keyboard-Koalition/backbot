...
func (c *AgnesController) HandleInbound(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := io.Copy(w, resp.Body); err != nil {
		log.Printf("agnes copy response body error: %v", err)
	}
...
func (c *AgnesController) HandleSend(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		http.Error(w, "method not allowed", http.StatusMethodNotAllowed)
		return
	}
...
func (c *AgnesController) HandleAgents(w http.ResponseWriter, r *http.Request) {
...
func (c *AgnesController) ExecuteCommand(ctx context.Context, cmd models.Command) (models.Response, error) {
	env := models.Envelope{
		SourceNode: c.name,
		DestNode:   cmd.Target,
		Payload:    nil,
...
func (c *AgnesController) validateInboundEnvelope(env *models.Envelope) error {
	if env.SourceNode == "" {
...
func (c *AgnesController) normalizeOutboundEnvelope(env *models.Envelope) error {
	if env.SourceNode == "" {
